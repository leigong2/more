...
import com.hyphenate.easeui.widget.chatrow.EaseChatRowInterAction;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    public EaseMessageAdapter(Context context, String username, int chatType, ListView listView, String nickName, String headerUrl) {
        this.context = context;
        this.listView = listView;
        toChatUsername = username;
        toNickName = nickName;
        toHeaderUrl = headerUrl;
    ...
    protected LinearLayout createChatRow(Context context, EMMessage message, int position) {
        LinearLayout chatRow = null;
        ...
        switch (message.getType()) {
            ...
            case LOCATION:
                ...
                double latitude = locBody.getLatitude();
                Log.i("zune:","lat = "+latitude);
                double longitude = locBody.getLongitude();
                Log.i("zune:","lng = "+longitude);
                String address = locBody.getAddress();
                Log.i("zune:","add = "+address);
        ...
        if (chatRow instanceof EaseChatRow) {
            ((EaseChatRow) chatRow).nickName = toNickName;
            ((EaseChatRow) chatRow).headerUrl = toHeaderUrl;
        }
        return chatRow;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        EMMessage message = getItem(position);
        //Todo 复用convertView
        int isSystemMessage = 0;
        try {
            isSystemMessage = message.getIntAttribute(EaseConstant.ISSYSTEMMESSAGE);
        } catch (HyphenateException e) {
            e.printStackTrace();
        }
        if (isSystemMessage == 1) {
            Log.i("zune:", "message (sys = 1) " + position + " = " + message);
            EaseChatRowInterAction chatRow = new EaseChatRowInterAction(context, message, position, this);
            chatRow.setUpView(message, position);
            return chatRow;
        } else {
            if (convertView == null||convertView instanceof EaseChatRowInterAction)
                convertView = createChatRow(context, message, position);

            //refresh ui with messages
            if (convertView instanceof EaseChatRow)
