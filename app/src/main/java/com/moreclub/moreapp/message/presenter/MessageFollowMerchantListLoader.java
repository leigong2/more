...
public class MessageFollowMerchantListLoader extends BasePresenter <IMessageFollowMerchantListLoader.LoaderMessageFollowMerchantDataBinder> implements IMessageFollowMerchantListLoader {
    @Override
    public void loadFollowsMerchant(String uid,String type,String pn,int followType) {
        Callback callback = new Callback<RespDto<ArrayList<FollowsUser>>>() {
            ...
            public void onResponse(Call<RespDto<ArrayList<FollowsUser>>> call,
                ...
                if (response!=null&&response.code() == 401){
                    if (getBinder() != null)
                        getBinder().onMessageFollowMerchantFailure("401");
                    return;
                }
                if (response.body().isSuccess()){
                    if (getBinder() != null)
                        getBinder().onMessageFollowMerchantResponse(response.body());
                }
            }
            ...
            public void onFailure(Call<RespDto<ArrayList<FollowsUser>>> call, Throwable t) {
                if (getBinder() != null)
                    getBinder().onMessageFollowMerchantFailure(t.getMessage());
            }
        };
