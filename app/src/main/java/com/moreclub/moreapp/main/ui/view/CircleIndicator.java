...
public class CircleIndicator extends LinearLayout {
    ...
    private CycleGalleryViewPager cViewpager;
    ...
    public void setCycleGalleryViewPager(CycleGalleryViewPager viewPager){
        cViewpager = viewPager;
        if (cViewpager != null && cViewpager.getAdapter() != null) {
            mLastPosition = -1;
            createIndicators();
            cViewpager.removeOnPageChangeListener(mInternalPageChangeListener);
            cViewpager.addOnPageChangeListener(mInternalPageChangeListener);
            mInternalPageChangeListener.onPageSelected(cViewpager.getCurrentItem());
        }
    ...
    private final OnPageChangeListener mInternalPageChangeListener = new OnPageChangeListener() {
        ...
        @Override public void onPageSelected(int position) {
            ...
            if (cViewpager.getAdapter() == null || cViewpager.getAdapter().getCount() <= 0) {
                return;
            }
    ...
    private DataSetObserver mInternalDataSetObserver = new DataSetObserver() {
        @Override public void onChanged() {
            ...
            int newCount = cViewpager.getAdapter().getCount();
            int currentCount = getChildCount();

            if (newCount == currentCount) {  // No change
                return;
            } else if (mLastPosition < newCount) {
                mLastPosition = cViewpager.getCurrentItem();
            } else {
                mLastPosition = -1;
            }
    ...
    @Deprecated public void setOnPageChangeListener(OnPageChangeListener onPageChangeListener) {
        ...
        cViewpager.removeOnPageChangeListener(onPageChangeListener);
        cViewpager.addOnPageChangeListener(onPageChangeListener);
    }

    private void createIndicators() {
        removeAllViews();
        int count = cViewpager.getAdapter().getCount();
        if (count <= 0) {
            return;
        }
