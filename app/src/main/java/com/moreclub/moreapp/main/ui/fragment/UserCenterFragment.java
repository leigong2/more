...
import com.moreclub.moreapp.ucenter.ui.activity.PackageCheckActivity;
import com.moreclub.moreapp.ucenter.ui.activity.PlayMoreActivity;
import com.moreclub.moreapp.ucenter.ui.activity.SettingActivity;
import com.moreclub.moreapp.ucenter.ui.activity.UseLoginActivity;
import com.moreclub.moreapp.ucenter.ui.activity.UserCollectionActivity;
...
import com.moreclub.moreapp.ucenter.ui.activity.UserPrivilegeActivity;
...
public class UserCenterFragment extends BaseFragment implements ObservableScrollViewCallbacks,
    ...
    protected void onInitView(Bundle savedInstanceState) {
        ...
        onInitData2Remote();
        mPresenter = LogicProxy.getInstance().bind(getLogicClazz(), this);
        initData();
        setupViews();
    }
    ...
    public void setupViews() {
        ...
        firstEnter = PrefsUtils.getBoolean(mContext, Constants.KEY_UCENTER_FIRST, true);
        toolbarHeight = ScreenUtil.dp2px(mContext, 28);
        //avatarTranY = ScreenUtil.dp2px(mContext, (317-110-32));
        totalTranX = ScreenUtil.getScreenWidth(mContext) / 2 - ScreenUtil.dp2px(mContext, 56);
        ...
        final String avatarUrl = TextUtils.isEmpty(PrefsUtils.getString(mContext, Constants.KEY_AVATAR, "")) ?
                PrefsUtils.getString(mContext, Constants.KEY_THIRD_AVATAR, "") : PrefsUtils.getString(mContext, Constants.KEY_AVATAR, "");
        ...
        if (firstEnter) {
            PrefsUtils.setBoolean(mContext, Constants.KEY_UCENTER_FIRST, false);
        } else {
        ...
        shareLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                 ...
                 * @param moduleID
                 */
                ShareHandler shareHandler = new ShareHandler(mContext, 2, "0");
                shareHandler.showShareView();
                shareHandler.shareTitle.setText("分享至");
                shareHandler.shareContent.setVisibility(View.VISIBLE);
                shareHandler.shareContent.setText("分享1次／天可得20里程，每日坚持，累计可得500里程，积累里程升级身份，享特权。");
    ...
    private void initData() {
        noReadCoupon = PrefsUtils.getBoolean(mContext, Constants.KEY_NO_READ_COUPON, false);
        loadData();
    }
    ...
    public void onUserCenterResponse(RespDto response) {
        ...
        if (userInfo != null && userInfo.getMerchantId() != null
                && userInfo.getMerchantId().length() > 0) {
    ...
    private void setupHScrollView() {
        ...
        if (userInfo != null) {
            ...
            if (userInfo.getCardLevel() == Constants.CARD_TYPE_NORMAL) {
                ...
                hScrollView.scrollBy(orangeScrollX * 2 + ScreenUtil.dp2px(mContext, 30), 0);
            }
        }
    ...
    private void setupMileAge() {
        if (userInfo != null) {
            ...
            if (userInfo.getCardLevel() == Constants.CARD_TYPE_NORMAL) {
                ...
                int scrollX = orangeScrollX * 2 + ScreenUtil.dp2px(mContext, 30);
    ...
    private void setupTopProgress() {
        if (userInfo.getCurMiles() <= ORANGE_CARD_LIMIT) {
            ...
            int tranX = ScreenUtil.getScreenWidth(mContext) * progress / ORANGE_CARD_LIMIT;
            ...
            tvMileAgeInfo.setTranslationX(tranX - measuredWidthTicketNum / 2 + ScreenUtil.dp2px(mContext, 6));
        } else if (userInfo.getCurMiles() < BLACK_CARD_LIMIT) {
            ...
            int tranX = ScreenUtil.getScreenWidth(mContext) * progress / BLACK_CARD_LIMIT;
            scrollPoint.setTranslationX(tranX);
            ...
            int tranX = ScreenUtil.getScreenWidth(mContext) * BLACK_CARD_LIMIT_MIN / BLACK_CARD_LIMIT;
            int spec = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
            tvMileAgeInfo.measure(spec, spec);
            int measuredWidthTicketNum = tvMileAgeInfo.getMeasuredWidth();
    ...
    private void setupStickViews() {
        if (userInfo != null) {
            ...
            PrefsUtils.setInt(mContext, Constants.KEY_UCENTER_FELLOWER, userInfo.getFollower());
            PrefsUtils.setInt(mContext, Constants.KEY_UCENTER_FOLLOW, userInfo.getFollow());
    ...
    void onClickFollowerCount() {
        ActivityOptionsCompat compat = ActivityOptionsCompat.makeCustomAnimation(
                mContext, R.anim.slide_right_in, R.anim.slide_right_out);
        Intent intent = new Intent(mContext, MessageFollowListActivity.class);
    ...
    void onClickPlayMore() {
        ...
        AppUtils.pushLeftActivity(mContext, PlayMoreActivity.class);
    }
    ...
    void onClickModifyUserInfo() {
        ...
        intent_merchant.putExtra("toUid", MoreUser.getInstance().getUid() + "");
        ActivityCompat.startActivity(mContext, intent_merchant, compat_merchant.toBundle());
//        AppUtils.pushLeftActivity(mContext, UserModifyActivity.class);
    }
    ...
    void onClickMyOrderLayout() {
        ...
        AppUtils.pushLeftActivity(mContext, MyOrderActivity.class);
    }
    ...
    void onClickMyCollectLayout() {
        ...
        AppUtils.pushLeftActivity(mContext, UserCollectionActivity.class);
    }
    ...
    void onClickMyPrivilegeLayout() {
        ...
        ActivityOptionsCompat compat = ActivityOptionsCompat.makeCustomAnimation(
                mContext, R.anim.slide_right_in, R.anim.slide_right_out);
        Intent intent = new Intent(mContext, UserPrivilegeActivity.class);
        if (userInfo != null) {
    ...
    void onClickCheckLayout() {
        ActivityOptionsCompat compat = ActivityOptionsCompat.makeCustomAnimation(
                mContext, R.anim.slide_right_in, R.anim.slide_right_out);
        Intent intent = new Intent(mContext, PackageCheckActivity.class);
        if (userInfo != null)
