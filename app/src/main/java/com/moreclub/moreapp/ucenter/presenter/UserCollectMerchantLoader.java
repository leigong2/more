...
public class UserCollectMerchantLoader extends BasePresenter<IUserCollectMerchantLoader.LoadUserCollectMerchantDataBinder> implements IUserCollectMerchantLoader{
    
    @Override
    public void loadMerchant(String uid,String type,String pageIndex) {
        Callback callback = new Callback<RespDto<ArrayList<CollectlistInfo>>>() {
            ...
            public void onResponse(Call<RespDto<ArrayList<CollectlistInfo>>> call,
                ...
                if (getBinder() != null && response!=null&&response.code() == 401){
                    if (getBinder() != null)
                        getBinder().onUserCollectMerchantFailure("401");
                    return;
                }
                if (response.body().isSuccess()){
                    if (getBinder() != null)
                        getBinder().onUserCollectMerchantResponse(response.body());
                }
            }
            ...
            public void onFailure(Call<RespDto<ArrayList<CollectlistInfo>>> call, Throwable t) {
                if (getBinder() != null)
                    getBinder().onUserCollectMerchantFailure(t.getMessage());
            }
        };
